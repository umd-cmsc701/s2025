...
> cat build.sh
#!/usr/bin/env bash
c++ -O3 -o fasta_stats fasta_stats.cpp
```
...
```
> tar czvf CMSC701_A0.tar.gz CMSC701_A0
```

This should create an output file called `CMSC701_A0.tar.gz` in the current directory, which is something you can upload.
...
Your program should write to `stdout` a **properly-formatted** JSON object (string) with the following information (using the following keys, each associated with the proper value):
